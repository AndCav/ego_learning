...
class EgoStayUpEnv(ego_env.egoEnv):
    def __init__(self):

        ros_ws_abspath = rospy.get_param("/ego/ros_ws_abspath", None)
        assert ros_ws_abspath is not None, "You forgot to set ros_ws_abspath in your yaml file of your main RL script. Set ros_ws_abspath: \'YOUR/SIM_WS/PATH\'"
        assert os.path.exists(ros_ws_abspath), "The Simulation ROS Workspace path " + ros_ws_abspath + \
                                               " DOESNT exist, execute: mkdir -p " + ros_ws_abspath + \
                                               "/src;cd " + ros_ws_abspath + ";catkin_make"

        ROSLauncher(rospackage_name="ego_description",
                    launch_file_name="ego_robot_noArm.launch",
        ...
        super(EgoStayUpEnv, self).__init__(control_type=self.control_type,
                                                ros_ws_abspath=ros_ws_abspath)

    def get_params(self):
        # get configuration parameters
        self.n_actions = rospy.get_param('/ego/n_actions')
        self.min_pole_angle = rospy.get_param('/ego/min_pole_angle')
        self.max_pole_angle = rospy.get_param('/ego/max_pole_angle')
        self.max_base_velocity = rospy.get_param('/ego/max_base_velocity')
        self.min_base_pose_x = rospy.get_param('/ego/min_base_pose_x')
        self.max_base_pose_x = rospy.get_param('/ego/max_base_pose_x')
        self.pos_step = rospy.get_param('/ego/pos_step')
        self.running_step = rospy.get_param('/ego/running_step')
        self.init_pos = rospy.get_param('/ego/init_pos')
        self.wait_time = rospy.get_param('/ego/wait_time')
        self.control_type = rospy.get_param('/ego/control_type')
    ...
    def _set_action(self, action):
        ...
        if action == 0:  # FORWARD
            ...
            self.pos[0] += self.pos_step
        elif action == 2:  # FORWARD BIG
            rospy.loginfo("GO FORWARD BIG...")
            self.pos[0] -= self.pos_step * 10
        elif action == 3:  # BACK BIG
            rospy.loginfo("GO BACK BIG...")
            self.pos[0] += self.pos_step * 10
            ...
            rospy.loginfo("GO BACK...")
            self.pos[0] += self.pos_step
        elif action == 6:  # FORWARD BIG
            rospy.loginfo("GO FORWARD BIG...")
            self.pos[0] -= self.pos_step * 10
        elif action == 7:  # BACK BIG
            rospy.loginfo("GO BACK BIG...")
            self.pos[0] += self.pos_step * 10
