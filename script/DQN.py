...
from ego_learning.openai_ros import stay_up
class DQNRobotSolver():
    def __init__(self, environment_name, n_observations, n_actions, n_episodes=1000, n_win_ticks=195, min_episodes= 100, max_env_steps=None, gamma=1.0, epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.995, alpha=0.01, alpha_decay=0.01, batch_size=64, monitor=False, quiet=False):
        ...
        if monitor: self.env = gym.wrappers.Monitor(self.env, '../data/ego-v0', force=True)
...
if __name__ == '__main__':
    rospy.init_node('Ego_n1try_algorithm', anonymous=True, log_level=rospy.FATAL)
    
    environment_name = 'EgoStandup-v0'
    
    n_observations = rospy.get_param('/ego/n_observations')
    n_actions = rospy.get_param('/ego/n_actions')
    
    n_episodes = rospy.get_param('/ego/episodes_training')
    n_win_ticks = rospy.get_param('/ego/n_win_ticks')
    min_episodes = rospy.get_param('/ego/min_episodes')
    max_env_steps = None
    gamma =  rospy.get_param('/ego/gamma')
    epsilon = rospy.get_param('/ego/epsilon')
    epsilon_min = rospy.get_param('/ego/epsilon_min')
    epsilon_log_decay = rospy.get_param('/ego/epsilon_decay')
    alpha = rospy.get_param('/ego/alpha')
    alpha_decay = rospy.get_param('/ego/alpha_decay')
    batch_size = rospy.get_param('/ego/batch_size')
    monitor = rospy.get_param('/ego/monitor')
    quiet = rospy.get_param('/ego/quiet')
